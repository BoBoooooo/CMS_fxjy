...
<%if(IsSearch==0){conn.Open();%><ul class="tw_articles tw_articles_dflm">
<% 
...
%>
  <li>
    ...
    <div class="con">
      <a href="<%=Sort_Url(int.Parse(dr["site_id"].ToString()),"article",int.Parse(dr["sort_id"].ToString()))%>" class="tw_flmc">[<%=Sort_Name(int.Parse(dr["sort_id"].ToString()))%>]</a>
      ...
      <div class="introduct"><%=dr["pa_introduct"]%></div>
    <div>
  </li>
<%
 }
