...
%>
<li>
  <span class="pic"><img src="<%=dr["titlepic"]%>" border="0" style="<%=TitlePic_Size%>"></span>
  <div class="con">
<span class="title"><%=SubStr(dr["title"].ToString(),Title_Num,true)%></span>
<div class="introduct"><%=dr["content"].ToString()%></div>
<a class="ljbm" href="/index.aspx?lanmuid=88&sublanmuid=707&prod=<%=Server.HtmlEncode(dr["title"].ToString())%>">立即报名></a>
  </div>
</li>
<%
 }
