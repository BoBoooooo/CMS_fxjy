...
<%if(i==0){%>
<li class="tw">
<%if(pic!=""){%><div class="xw"><a href="<%=Detail_Url(dr)%>"><%=SubStr(dr["title"].ToString(),Title_Num,true)%></a></div>
<div class="date"><%=((DateTime)dr["thedate"]).ToString("yyyy-MM-dd")%></div>
<p><%=SubStr(dr["pa_introduct"].ToString(),160,true)%></p>
<%}
else
{%>
<div class="xw"><a href="<%=Detail_Url(dr)%>"><%=SubStr(dr["title"].ToString(),Title_Num,true)%></a></div>
<div class="date"><%=((DateTime)dr["thedate"]).ToString("yyyy-MM-dd")%></div>
<p><%=SubStr(dr["pa_introduct"].ToString(),160,true)%></p>
<%
}
...
{%>
<li class="lb">
<span class="tits"><a href="<%=Detail_Url(dr)%>"><%=dr["title"].ToString()%></a></span>
<span class="dates"><%=((DateTime)dr["thedate"]).ToString("yyyy-MM-dd")%></span>
